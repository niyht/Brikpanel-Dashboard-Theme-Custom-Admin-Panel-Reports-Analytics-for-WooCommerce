...
function brikpanel_get_developer_hooks() {
    return [
        ...
        [
            ...
            'description' => __( 'Register additional columns in the BrikPanel modern products list. A definition takes a label, a render callback and an optional width (an integer for pixels, or a CSS length such as "15%"). Only definitions carrying a render callback are drawn; they appear after the built-in columns and before the row actions, and are listed under "Plugin columns" in the Columns picker. The callback receives the product and its ID and returns the cell HTML. For a variable product it receives the parent, so you can aggregate across its variations. Cell output is sanitised the same way third-party column cells are, a callback that echoes instead of returning also works, and a callback that throws leaves an empty cell instead of breaking the list. New columns are visible by default; add "default" => false to make them opt-in. These columns are display-only: they cannot be sorted and are not part of CSV exports.', 'brikpanel' ),
            'example'     => "add_filter( 'brikpanel_products_columns', function ( \$cols, \$user_id ) {\n    \$cols['vendor'] = [\n        'label'  => __( 'Vendor', 'my-plugin' ),\n        'width'  => 120,\n        'render' => function ( \$product, \$product_id ) {\n            \$vendor = get_post_meta( \$product_id, '_vendor', true );\n\n            return \$vendor ? esc_html( \$vendor ) : '';\n        },\n    ];\n    return \$cols;\n}, 10, 2 );",
        ],
        [
        ...
        [
            'name'        => 'brikpanel_cartab_email_captured',
            'type'        => 'action',
            'signature'   => 'do_action( "brikpanel_cartab_email_captured", array $entry )',
            'description' => __( 'Fires when a NEW email lands in the abandoned-cart list (typed at checkout, submitted in the signup popup, or auto-captured from a logged-in account). $entry contains email, name, source, status, the decoded cart snapshot and timestamps.', 'brikpanel' ),
            'example'     => "add_action( 'brikpanel_cartab_email_captured', function ( \$entry ) {\n    // e.g. subscribe the address to your ESP\n    my_esp_subscribe( \$entry['email'], \$entry['first_name'] );\n} );",
        ],
        [
            'name'        => 'brikpanel_cart_abandoned',
            'type'        => 'action',
            'signature'   => 'do_action( "brikpanel_cart_abandoned", array $entry )',
            'description' => __( 'Fires when a captured cart is marked abandoned (no activity for the configured number of minutes). This is the hook a follow-up email plugin subscribes to. $entry includes the cart snapshot with per-item variation attributes.', 'brikpanel' ),
            'example'     => "add_action( 'brikpanel_cart_abandoned', function ( \$entry ) {\n    my_mailer_queue_abandoned_cart_email( \$entry['email'], \$entry['cart_items'], \$entry['cart_total'] );\n} );",
        ],
        [
