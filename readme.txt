...
= 3.1.25 (2026-06-15) =
* Fix: **The modern Products screen now respects brand, tag and other taxonomy filters from the product count links**. On a product taxonomy screen (such as Brands or Tags), clicking the product count next to a term opens a filtered product view. The modern Products screen was ignoring that filter and showing every product instead. It now opens already filtered to that term, shows a removable filter chip (for example "Brand: Aurora") so it is clear which filter is active, and clearing the chip restores the full list. Works for brands, tags, attributes and any custom product taxonomy, on both simple and variable products. Reported by a user
...
= Earlier versions =
For the complete release history (3.1.19 and all earlier versions, back to 1.2.0), see changelog.txt bundled with the plugin.
