...
== Changelog ==
= 3.3.1 (2026-09-10) =
* New: **The abandoned cart popup can now be written separately for each of your languages.** Its wording lived in one set of boxes, so a store selling in Estonian and Finnish showed the same text on both. On a store running Polylang, WPML, TranslatePress or Weglot a "Popup text per language" card now appears under WooCommerce → Settings → BrikPanel → Abandoned Carts, with a tab for each language; anything you leave empty falls back to the wording above it. The wording the popup writes on its own is included as well (the discount code heading, the Copy button, the "OFF" badge, the error messages), so a language BrikPanel ships no translation for no longer ends up half in English. A store with one language sees no change.
* Fix: **Add-to-cart figures are no longer inflated by scripted traffic.** The "one visitor, one product, once a day" rule was kept in the WooCommerce session, so a script that dropped its cookies between rounds arrived with a blank memory and was counted again every round; one store recorded 13,053 add-to-carts in a month where the real level was four to seven a day. The rule is now held on the server too, where a visitor cannot clear it, and it covers all three figures: per product, store-wide and on the checkout page. Signed-in customers are unaffected. Two guests sharing one connection and the same browser make-up who add the same product on the same day are now counted once.
* New: **BrikControl can put right add-to-cart history that was already inflated.** A new "Add-to-Cart History" check lists days that cannot be real (more add-to-carts than the store had visitors) and days far above that product's own normal, and "Correct these days" lowers them to an honest ceiling. Nothing is deleted, your products, orders and customers are untouched, and "Undo last correction" puts the earlier figures back exactly.
* New: **BrikControl now reports abandoned cart entries that look automated.** A new "Abandoned Cart Entries" check looks for four shapes a shopper's browser does not produce: an entry carrying a browser id BrikPanel never issued, one browser id typing many addresses within minutes, one address arriving from many browser ids, and dozens of empty signups inside a single minute. It only reports. Nothing is deleted and there is no button, because a shared computer or a busy promotion can look much the same.
* Fix: **The abandoned cart signup endpoint is far harder to flood.** It accepted any browser id the caller sent and tied its two-second limit to that same id, so a script that changed its cookie on every request wrote a row every time. An id BrikPanel did not issue is now refused and replaced, and the limit follows the connection as well as the cookie: a cookie-swapping script drops from a row per request to one in five, while real shoppers, and several people behind one connection, carry on as before.

= 3.3.0 (2026-09-07) =
* New: **BrikMentor is out.** BrikMentor is our separate paid add-on that acts on the figures BrikPanel shows you: it chases abandoned carts, wins lapsed customers back and sends campaigns to your customer segments, by email, from inside WooCommerce. Now that it is available, the "coming soon" wording is gone and BrikPanel points at the real thing: a card on the dashboard, a row under WooCommerce > Settings > BrikPanel, a button on Customer Analytics and Abandoned Carts that explains what BrikMentor would do with the screen you are looking at, and a single one-time note announcing the launch. The first month is $1. Nothing is ever installed from inside your admin: the buttons open BrikMentor's own page, where you buy it and download it yourself. Every one of these is dismissible, none of them appear if BrikMentor is already installed here, and a brand new store is left alone for its first three days.
* New: **The BrikMentor waiting list is now a plain newsletter.** It used to offer a place in a private beta, which no longer exists now that BrikMentor is out. It offers what it always really was: an occasional short email about new features, WooCommerce tips and ideas for growing your store. The sign-up box no longer opens by itself when your store passes 100 or 200 orders; it opens only when you press Subscribe, on the dashboard or under WooCommerce > Settings > BrikPanel. It is two steps instead of three, since it no longer asks which email tool you use, and the cards it used to place on Customer Analytics and Abandoned Carts are gone. If you are already on the list you stay on it and need do nothing.
* Fix: **The X on the Store Health warning now closes it for good.** It was written as a seven-day snooze, so the warning you had dismissed came back by itself a week later. Dismissing now sticks for the problems it was showing you, and the banner only speaks up again if a different check turns critical. Two things that made this worse are fixed with it: the X did nothing at all while the top bar was switched on, and a dismissal the server refused still cleared the banner from the screen without being saved anywhere, so it returned on the next page load. If saving fails you are now told. On a multisite network, dismissing on one store no longer silences the warning on all the others.
* Fix: **No more repeated deprecation lines in the error log on WooCommerce 11.1.** Drawing the notifications bell asked WooCommerce whether its Analytics feature was switched on, in a way WooCommerce retired in 11.1, and that wrote a line to the log on every admin page load. The question is now asked the way 11.1 expects. The "New customers today" entry still opens the right screen, with Analytics on or off.

= 3.2.99 (2026-09-07) =
* Fix: **Exported CSV files can no longer run anything when you open them.** Spreadsheet programs treat a cell starting with `=`, `+`, `-` or `@` as a formula rather than as text, so wording a customer typed at checkout could be run by Excel or Google Sheets the moment you opened an export. Placing a guest order was enough to plant it, and it ran on your computer, not the store's. Only the phone column was protected, and only in two of the exports. Every column of every CSV export is now protected: orders and customers under Segments, the lifetime value, RFM and cohort files under Customer Analytics, abandoned carts, selected products, and operational expenses. Plain numbers are left alone, so a negative stock level or a refund still reads as a number and still sorts. Excel downloads were never affected and are unchanged.
* Fix: **An expense whose title contains a comma no longer breaks the exported file.** In the operational expenses export the date, title and amount were written without quoting, so a single comma in a title pushed every following column one place to the right for that row. The file also carries the same marker as the other exports now, so Turkish characters open correctly in Excel instead of as symbols.
