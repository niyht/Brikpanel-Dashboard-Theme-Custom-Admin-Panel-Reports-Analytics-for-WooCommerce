/**
 * Collapsible per-status settings cards.
 *
 ...
 *
 * Two features share this markup — the status-change emails card and the
 * WhatsApp per-status drafts card — so every `.bp-cse-card` on the page is bound
 * separately. They must not share a "last focused field": a token chip clicked
 * in one card would otherwise drop its text into the other.
 */
( function () {
	'use strict';
	...
	var cards = document.querySelectorAll( '.bp-cse-card' );
	if ( ! cards.length ) {
		return;
	}
	...
	function bindCard( card ) {
		// Remember the last-focused insertable field in this card, so a
		// placeholder chip knows where to drop its token (defaults to the
		// item's Message textarea).
		var lastField = null;
