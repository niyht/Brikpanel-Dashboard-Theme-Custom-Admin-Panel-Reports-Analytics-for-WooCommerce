<?php
/**
 * BrikPanel — Sheets Logger (ring buffer + redaction).
 *
 ...
 * and authorization codes so secrets never land in wp_options.
 *
 * @package BrikPanel
 * @since   2.9.0
 */
...
class Brikpanel_Sheets_Logger {

	/** Option key holding the ring buffer. autoload=no. */
	const OPTION = 'brikpanel_gs_error_log';
	 ...
	 *
	 * @param string $flow    One of: oauth, orders, reports, customers, client.
	...
	public static function log( $flow, $message, $code = 0, array $context = [] ) {
		...
		// autoload=no — this option is only read on the settings page.
		update_option( self::OPTION, $log, false );
	}

	/**
	 * Convenience: log an error from a wp_remote_* failure or a Sheets API
	 * error response body.
	 *
	 * @param string                $flow
	 ...
	 *  - "code":"4/0Ad..." (OAuth auth codes)
	 *  - Raw long ya29.* access token shapes
	...
	public static function redact( $msg ) {
		...
		// JSON-looking access_token / refresh_token / id_token / code values.
		$msg = preg_replace(
			'/"(access_token|refresh_token|id_token|code)"\s*:\s*"[^"]{6,}"/i',
			'"$1":"[REDACTED]"',
			$msg
		);

		// Raw Google token shapes (best-effort): ya29.<base64-ish>
		$msg = preg_replace( '/\bya29\.[A-Za-z0-9._\-]{20,}/', '[REDACTED-ya29]', $msg );

		// 1// refresh token shape.
		$msg = preg_replace( '#\b1//[A-Za-z0-9._\-]{20,}#', '[REDACTED-rt]', $msg );

		// 4/0A... auth code shape.
